 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: l_video_trans.c,v 1.14 1999/10/12 13:01:11 cphipps Exp $
 *
...
#ifndef lint
static const char rcsid[] = "$Id: l_video_trans.c,v 1.14 1999/10/12 13:01:11 cphipps Exp $";
#endif /* lint */
...
static void I_Double8Buf(pval* out_buffer, const char* src)
{
  register unsigned int *olineptr = out_buffer;
  register const unsigned int *ilineptr = (const unsigned int*)src;
  int y = SCREENHEIGHT;
  ...
  do {
    ...
    do {
      ...
      unsigned int* oline2ptr = olineptr + SCREENWIDTH*2 / sizeof(*olineptr);
#ifndef I386
...
static void I_Triple8Buf(pval* out_buffer, const char* src)
{
  register unsigned int *olineptr = out_buffer;
  const unsigned int *ilineptr = (const unsigned int*)src;
  int x, y;
...
static void I_Copyto16Buf(pval* out_buffer, const char* src)
{
  const char* iptr = src;
  unsigned short* optr = (unsigned short*)out_buffer;
  int i,j;
...
static void I_Double16Buf(pval* out_buffer, const char* src)
{
  register const char* iptr = src;
  register unsigned int* optr = out_buffer;
  int y = SCREENHEIGHT;
  static unsigned int* pixelpair = NULL;
  ...
  do {
    ...
    do {
      unsigned int* optr2 = optr + 4*SCREENWIDTH / sizeof(*optr);
...
#define LONG_AT(p) *((unsigned int*)(p))

static byte buffor24bpp[4*3+1];

static void I_Copyto24Buf(pval* out_buffer, const char* src)
{
#ifndef I386
...
static void I_Double24Buf(pval* out_buffer, const char* src)
{
  register const char* iptr = src;
  register unsigned char* optr = (unsigned char*)out_buffer;
...
static void I_Copyto32Buf(pval* out_buffer, const char* src)
{
  register const char* iptr = src;
  register unsigned int* optr = (unsigned int*)out_buffer;
...
static void I_Double32Buf(pval* out_buffer, const char* src)
{
  register const char* iptr = src;
  register unsigned int* optr = (unsigned int*)out_buffer;
  int y = SCREENHEIGHT;
...
typedef void (*expand_func_t)(pval*, const char*);

void (*I_ExpandImage)(pval* dest, const char* src);
