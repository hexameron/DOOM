 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: d_main.c,v 1.42 1999/10/17 09:35:15 cphipps Exp $
 *
...
static const char rcsid[] = "$Id: d_main.c,v 1.42 1999/10/17 09:35:15 cphipps Exp $";
  ...
  do
    {
      ...
      I_FinishUpdate(1);            // wipes not in 3d
    }
  while (!done);
}
...
void D_Display (void)
{
  ...
  // normal update
  if (!wipe)
    I_FinishUpdate (rightFrame);              // page flip or blit buffer
  else {
    // wipe update
  ...
  for (;;)
    {
      ...
      D_Display();
      rightFrame = 1 ;
      D_Display(); // two frames for stereo 3d
  ...
  if (!base)        // cache multiple requests
    {
      ...
      base = malloc(len + strlen(lxdoom_dir));
      strcpy(base, home);
...
static char *FindIWADFile(void)
{
  ...
  *customiwad = 0; // customiwad is blank
  ...
  // CPhipps - use same DOOMWADDIR env. variable as DeuTex
  if ((p = getenv("DOOMWADDIR"))) {
    strcpy(iwad,p);
  ...
  // CPhipps - use wad dir formed during configuration
  strcpy(iwad,DOOMWADDIR);
  //jff 9/3/98 use logical output routine
  lprintf(LO_INFO,"Looking in %s\n",iwad);
  if (*customiwad)
  {
...
void D_DoomMainSetup(void)
{
...

  {
    ...
    switch ( gamemode ) {
    case retail:
      doomverstr = "The Ultimate DOOM Startup";
      break;
    case shareware:
      doomverstr = "DOOM Shareware Startup";
      break;
    case registered:
      doomverstr = "DOOM Registered Startup";
      break;
  ...
  if (!M_CheckParm("-noload")) {
	...
	for (i=0; i<4; i++) {
	  ...
	  path_buf = (char*)malloc(strlen(fname)+2+((folder == NULL) ? 0 : strlen(folder)));
	  
	  if (folder == NULL)
	    strcpy(path_buf,fname);
	  else {
	    strcpy(path_buf,folder);
	  ...
	  if (!access(path_buf, R_OK)) {
	    ...
	    const char* ext = NULL;
	    if (strlen(path_buf)>=4) ext = path_buf + strlen(path_buf) - 4;
	    if (ext && (!strcasecmp(ext,".deh") || !strcasecmp(ext,".bex"))) 
	      ProcessDehFile(path_buf, D_dehout(), 0);
	    else {
	      D_AddFile(path_buf,source_auto_load);
	      modifiedgame = true; 
	      i=99; // Exit loop
	    }
	  }
